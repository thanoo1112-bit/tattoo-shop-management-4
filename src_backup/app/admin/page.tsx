...
export default async function AdminDashboard() {
  ...
  // Calculate actual deposit revenue from the database
  const depositRevenue = appointments?.reduce((sum, app) => sum + (Number(app.deposit_amount) || 0), 0) || 0;

  const STATS = role === 'admin' ? [
    { label: "รายได้มัดจำ (Deposit Revenue)", value: `฿${depositRevenue.toLocaleString()}`, icon: CalendarClock, color: "text-emerald-400" },
    { label: "คิวจองทั้งหมด (Total Bookings)", value: totalBookings, icon: CalendarClock, color: "text-blue-400" },
    { label: "รอตรวจ Reference (Pending)", value: pendingCount, icon: Clock, color: "text-yellow-400" },
    { label: "อนุมัติแล้ว (Approved)", value: approvedCount, icon: CheckCircle, color: "text-green-400" },
  ] : [
    { label: "คิวงานส่วนตัว (Your Bookings)", value: totalBookings, icon: CalendarClock, color: "text-blue-400" },
    { label: "รอตรวจ Reference (Pending)", value: pendingCount, icon: Clock, color: "text-yellow-400" },
    { label: "อนุมัติแล้ว (Approved)", value: approvedCount, icon: CheckCircle, color: "text-green-400" },
    { label: "งานสักเสร็จสิ้น (Completed)", value: completedCount, icon: CheckCircle, color: "text-purple-400" },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-gothic tracking-widest uppercase">
            {role === 'admin' ? 'ภาพรวมร้าน (ADMIN DASHBOARD)' : 'หน้าจัดการคิว (ARTIST DASHBOARD)'}
          </h1>
